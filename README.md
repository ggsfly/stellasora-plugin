...
| **排行榜查询** | `/st_bb`・`/st_fe` 分别查询当期联合讨伐/终焉绝响国服 Top100，触发时按赛季指针拉取，瘦身落盘 + TTL 缓存 |
...
聊天中发送 `/st_bb`（联合讨伐）或 `/st_fe`（终焉绝响）——所有人可用、受黑白名单约束，返回当期赛季排行榜**国服 Top 100**，纯文本回复。两榜独立成命令，避免单条消息过长。
