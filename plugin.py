...
class StellaSoraPlugin(MaiBotPlugin):
    ...
    _DIRECT_SEND_PROMPT = (
        ...
        "3. 数值、等级必须与资料完全一致\n"
        "4. 资料按「配队N（队伍名）→ 本角色（定位）→ 队友（定位）」分块。**本角色**"
        "（用户问询的角色）按问法输出字段；**队友一律只报名字和定位，各占一行，"
        "禁止展开队友的描述/技能/秘纹/纹章**。本角色字段裁剪规则：\n"
        "   a. 用户问题含「完整/详细/全部/所有」→ 全量输出：每个队伍的全部字段，"
        "不限字数；同一字段在多个队伍中内容相同时只详述一次，"
        "其余队伍注明\"与配队N相同\"即可（去重不删内容）\n"
        "   b. 用户只问「配队」→ 本角色只输出：定位和描述（不输出技能升级优先度、"
        "秘纹和纹章）\n"
        "   c. 用户只问「纹章」→ 本角色只输出：纹章推荐\n"
        "   d. 用户只问「秘纹」→ 本角色只输出：推荐主位秘纹\n"
        "   e. 用户只问「技能/升级」→ 本角色只输出：技能升级优先度\n"
        "   f. 其他笼统问法（如只说\"XX攻略\"）→ 本角色输出：描述、技能升级优先度、"
        "纹章推荐（不超过 1500 字；若队伍较多则优先输出最相关的 1-2 个队伍）\n"
